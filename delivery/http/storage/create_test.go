...
import (
	...
	"github.com/teq-quocbang/arrows/util/test"
...
func SetUpForeignKeyData(db *database.Database) (uuid.UUID, uuid.UUID, uuid.UUID, string, error) {
	...
	if productResp.Code != 200 {
		return uuid.UUID{}, uuid.UUID{}, uuid.UUID{}, "", fmt.Errorf("failed to create product, error: %v", producerResp.Body)
	}
	...
	if storageResp.Code != 200 {
		return uuid.UUID{}, uuid.UUID{}, uuid.UUID{}, "", fmt.Errorf("failed to upsert storage, error: %v", producerResp.Body)
	}
